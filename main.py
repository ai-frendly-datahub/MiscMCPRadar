...
def run(
    *,
    ...
    enabled_source_count = sum(1 for source in effective_sources if source.enabled)
    ...
    recent_articles: list[Article] = storage.recent_articles_by_collected_at(
        ...
        days=recent_days,
        limit=max(200, per_source_limit * max(len(effective_sources), 1) * 2),
    )
