# MiscMCPRadar - 기타 MCP 레이더

**🌐 Live Report**: https://ai-frendly-datahub.github.io/MiscMCPRadar/


Miscellaneous 섹션의 한국 MCP 서버 목록을 수집하고 리스크/활성도를 추적하는 독립 Radar입니다.

## 프로젝트 목표

- **데이터 수집**: RSS 피드 및 API
- **엔티티 분석**: 도메인별 키워드 매칭
- **트렌드 리포트**: DuckDB 저장 + HTML 리포트로 기타 MCP 동향 시각화
...
2. 실행:
   ```bash
   python main.py --category misc_mcp --recent-days 7
   # 리포트: reports/misc_mcp_report.html
   ```
...
- 워크플로: `.github/workflows/radar-crawler.yml`
  ...
  - 환경 변수 `RADAR_CATEGORY`는 `misc_mcp`로 고정합니다.
...
- 설정 방법:
  ...
  3) 워크플로 파일의 `RADAR_CATEGORY`가 `misc_mcp`인지 확인
...
- GitHub Pages visualization: `reports/index.html` (valid HTML); https://ai-frendly-datahub.github.io/MiscMCPRadar/.
